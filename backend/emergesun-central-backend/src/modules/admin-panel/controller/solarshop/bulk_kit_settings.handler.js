...
/**
 * GET /solarshop/bulk-kit-settings
 * List bulk kit settings filtered by country / state / cluster.
 */
const get_bulk_kit_settings = async (req, res) => {
    try {
        ...
        if (country_id) {
            ...
            if (isIndia) {
                return res.status(400).json({
                    status: 'error',
                    message: 'Use the India endpoint to retrieve India configurations.',
                });
            }
        ...
        console.error("Error in get_bulk_kit_settings:", error);
        res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

/**
 * GET /solarshop/bulk-kit-settings/warehouse/:warehouseId
 * List bulk kit settings for a single warehouse.
 */
const get_warehouse_bulk_settings = async (req, res) => {
    try {
        ...
        console.error("Error in get_warehouse_bulk_settings:", error);
        res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

/**
 * POST /solarshop/bulk-kit-settings/save
 * Upsert bulk kit settings for a single (warehouse, combo_kit) pair.
 */
const save_bulk_kit_settings = async (req, res) => {
    try {
        const {
            country_id,
            state_id,
        ...
        if (isIndia) {
            return res.status(400).json({
                status: 'error',
                message: 'India country configurations must be saved in the India database.',
            });
        }
        ...
        // Normalize tiers (and allow legacy "allowed_quantities" only payloads)
        ...
        if (is_bulk_enabled) {
            ...
            // Margin validation: each tier margin should not exceed previous one
        ...
        console.error("Error in save_bulk_kit_settings:", error);
        res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

/**
 * POST /solarshop/bulk-kit-settings/delete
 * Soft-delete a bulk kit settings row by id.
 */
const delete_bulk_kit_settings = async (req, res) => {
    try {
        const { id } = req.body;
        ...
        console.error("Error in delete_bulk_kit_settings:", error);
        res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

module.exports = {
    get_bulk_kit_settings,
    get_warehouse_bulk_settings,
    save_bulk_kit_settings,
    delete_bulk_kit_settings,
};
