...
const migrateUrl = async (url, targetFolder) => {
  ...
  if (isLocal) {
    ...
    oldCloudinaryInfos.push({
      public_id: `emergesun/${dirPath}/${filename}`,
      resource_type: resourceType
    });
    if (dirPath.startsWith('public/')) {
      const withoutPublic = dirPath.replace('public/', '');
      oldCloudinaryInfos.push({
        public_id: `emergesun/${withoutPublic}/${filename}`,
        resource_type: resourceType
      });
    } else {
      oldCloudinaryInfos.push({
        public_id: `emergesun/public/${dirPath}/${filename}`,
        resource_type: resourceType
      });
    }
  } else {
...
const migrateWarehouseImages = async () => {
  const targetFolder = 'emergesun/emergesun-warehouse-panel-backend/public/uploads/warehouse_docs';
