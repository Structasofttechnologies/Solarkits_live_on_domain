...
const get_supplier = async (req, res) => {
    try {
        ...
        // Fetch warehouses (no cross-connection .populate — manual district lookup below)
        const warehouses = await SupplierWarehouse.find({ supplier_id: req.params.id })
            .sort({ created_at: -1 })
            .lean();

        // Resolve supply_districts manually (geolocation_level_2 lives on geolocation_db,
        // not supplier_db, so Mongoose's .populate() cannot cross connections)
...
const approve_supplier = async (req, res) => {
    try {
        ...
        try {
            await send_mail(
                supplier.email,
                'EmergeSun Supplier Account Approved',
                `
                <div style="font-family: Arial, sans-serif; padding: 32px; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 16px;">
                  ...
                  <p style="font-size: 16px; color: #374151; line-height: 1.5;">Your application for the EmergeSun Supplier Ecosystem has been approved.</p>
                  ...
                  <p style="font-size: 11px; color: #9ca3af; text-align: center;">EmergeSun Solar Ecosystem — Supplier Portal</p>
                </div>
                `
            );
        } catch (mailErr) {
