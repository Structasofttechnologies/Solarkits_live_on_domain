...
const send_register_email_otp = async (req, res) => {
    try {
        ...
        let sent = false;
        try {
            const result = await sendOTP(emailLower, 'Verify Your EmergeSun Business Email', 'Your email verification OTP code is:');
            otp = result.otp;
            sent = true;
        } catch (mailErr) {
...
const request_verify_account_otp = async (req, res) => {
    try {
        ...
        const { otp } = await sendOTP(supplier.email, 'Verify Your EmergeSun Supplier Account', 'Your OTP verification code is:');
...
const request_forgot_password_otp = async (req, res) => {
    try {
        ...
        let sent = false;
        if (email) {
            const result = await sendOTP(supplier.email, 'Reset Your EmergeSun Supplier Passcode', 'Your password reset OTP is:');
            otp = result.otp;
            sent = true;
        } else {
            try {
    ...
    try {
        ...
        return res.status(200).json({
            ...
            message: purpose === 'verify'
                ? 'Account verified and passcode set. Welcome to EmergeSun!'
