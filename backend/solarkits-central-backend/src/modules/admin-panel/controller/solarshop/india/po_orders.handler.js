...
const { PurchaseOrder, WarehouseComboKit, CompanyMargin } = require('../../../models/india_solarshop_db');
const { ProductSkuPrice } = require('../../../models/core_db');
...
const create_po_order_india = async (req, res) => {
    try {
        ...
        if (!isIndia) {
            return res.status(400).json({ status: 'error', message: 'Non-India PO orders must be placed using the core API endpoint.' });
        }
        ...
        // 7. Trigger cached pricing calculation to make sure ComboKit caches are updated
        computeKitPrices(combo_kit_id, warehouse_id, true).catch(err => {
            console.error('[create_po_order_india] Async cache update error:', err);
        });

        return res.status(201).json({
            status: 'success',
            message: 'Purchase Order created and prices snapshotted successfully in India DB.',
            data: newPo
        });
    } catch (error) {
        console.error("Error in create_po_order_india:", error);
        return res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

const get_po_orders_india = async (req, res) => {
    try {
        ...
        console.error("Error in get_po_orders_india:", error);
        return res.status(500).json({ status: 'error', message: error.message || 'Internal Server Error' });
    }
};

module.exports = {
    create_po_order_india,
    get_po_orders_india
};
