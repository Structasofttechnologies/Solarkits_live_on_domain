...
// e.g. "https://res.cloudinary.com/xxx/image/upload/v123/solarkits/public/uploads/brands/FILE_abc"
// → { public_id: "solarkits/public/uploads/brands/FILE_abc", resource_type: "image" }
// -------------------------------------------------
...
function upload_files(upload_dir, max_size_mb, field_name, files_length) {
  ...
  const folder = `solarkits/solarkits-admin-panel-backend/${upload_dir.replace(/\\/g, "/")}`;
...
// -----------------------------------------------
// Dynamic Cloudinary Uploader for any files (fields)
// upload_dir is used to derive Cloudinary folder:
//   "public/uploads/combo_kits" → "solarkits/solarkits-admin-panel-backend/public/uploads/combo_kits"
// -----------------------------------------------
function upload_any_files(upload_dir, max_size_mb) {
  const folder = `solarkits/solarkits-admin-panel-backend/${upload_dir.replace(/\\/g, "/")}`;
    ...
    upload.any()(req, res, async (err) => {
      ...
      try {
        for (const file of req.files) {
          ...
          let prefix = "FILE";
          if (file.fieldname === "kit_image" || folder.includes("solar_kits") || folder.includes("combo_kits")) {
            prefix = "KIT";
          }
          if (file.fieldname.startsWith("bos_kit_image_")) {
            prefix = "BOS";
          }
          const public_id_name = `${prefix}_${unique}`;
