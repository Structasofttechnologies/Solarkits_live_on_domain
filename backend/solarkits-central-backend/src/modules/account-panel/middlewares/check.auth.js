...
const check_auth = async (req, res, next) => {
  try {
    ...
    if (!role.is_active || role.is_deleted) {
      return res.status(403).json({ status: "error", message: "User role is inactive or deleted." });
    }
    ...
    if (!department.is_active || department.is_deleted) {
      return res.status(403).json({ status: "error", message: "User department is inactive or deleted." });
    }

    const isSuperAdmin = role.name === 'Super Admin' || department.level === 'global';

    // 5. Validate requested panel exists
    const requestedPanelPrefix = '/account-panel';
    ...
    if (!isSuperAdmin) {
      ...
      if (!deptPanelLink) {
        return res.status(403).json({ status: "error", message: "Department does not have access to this panel." });
      }

      // 7. Validate panel assigned to role
      const rolePanelLink = await RolePanel.findOne({ role_id: role._id, panel_id: panel._id }).lean();
      if (!rolePanelLink) {
        return res.status(403).json({ status: "error", message: "Role does not have access to this panel." });
      }
    }
