...
import { MdExpandMore, MdExpandLess, MdLock } from "react-icons/md";
...
export default function Drawer({ isOpen, setIsOpen, isMobile, menuItems }) {
  ...
  const handleClick = (item, depth) => {
    if (item.is_locked) {
      // Logic for showing upgrade prompt or modal can go here
      return;
    }
    if (item.subMenu) {
  ...
  const renderMenuItems = (items, depth = 0) => (
    <ul className={`${depth > 0 ? "ps-2 mt-1 space-y-1" : "px-2 space-y-1"}`}>
      {items.map((item) => {
        ...
        const isMenuOpen = openMenus[item.name]?.open;
            ...
            <li>
              {isLocked || hasSub ? (
                <motion.button
                  ...
                  onClick={() => handleClick(item, depth)}
                  className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl transition-all duration-300 whitespace-nowrap group relative
                    ${isActive
                      ? "gradient-primary text-white shadow-md shadow-primary/20"
                      : isLocked 
                        ? "text-text-muted/50 bg-transparent cursor-not-allowed grayscale"
                ...
                >
                  ...
                  >
                    <span
                      className={`text-xl min-w-6 flex justify-center transition-colors ${isActive ? "text-white" : isLocked ? "text-text-muted/30" : "text-primary group-hover:scale-110"
                        }`}
                    >
                      {item.icon}
                    </span>
                    <span className={`flex-1 font-semibold text-[13.5px] tracking-tight truncate overflow-hidden text-ellipsis whitespace-nowrap ${isActive ? "text-white" : isLocked ? "text-text-muted/50" : "text-text-primary"}`}>
                      {item.name}
                    </span>
                  </div>
                  ...
                  {isLocked ? (
                    ...
                    <span className={`text-lg flex justify-center transition-transform duration-300 ${isMenuOpen ? "rotate-180" : ""}`}>
                      <MdExpandMore />
                    </span>
                  )}

                  {isLocked && (
                    <div className="absolute inset-0 bg-surface/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-xl">
                      <div className="bg-surface/90 px-2 py-1 rounded-md border border-border shadow-sm">
                        <p className="text-[10px] font-bold text-primary uppercase">Upgrade Plan</p>
                      </div>
                    </div>
                  )}
                </motion.button>
          ...
          >
            ...
            <div className="p-4 border-t border-border bg-gradient-to-t from-primary/5 to-transparent">
              <div className="bg-surface-hover rounded-2xl p-4 border border-border/50 group cursor-pointer hover:border-primary/30 transition-all duration-300">
                <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Supplier Portal</p>
