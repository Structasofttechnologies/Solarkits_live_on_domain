...
export default function FrontPage() {
    ...
    const [erpModulesConfig, setErpModulesConfig] = useState({
    ...
    const [happyUsersConfig, setHappyUsersConfig] = useState({
        sectionTitle: 'Our Happy Users',
        subTitle: 'Trusted by solar businesses across India',
        enableSection: true,
        stats: [
            { label: 'Active Users', value: '5000+', icon: 'Users', color: 'text-blue-500 bg-blue-50', status: 'Active' },
            { label: 'Companies', value: '1000+', icon: 'Briefcase', color: 'text-green-500 bg-green-50', status: 'Active' },
            { label: 'Projects Managed', value: '15000+', icon: 'Sun', color: 'text-orange-500 bg-orange-50', status: 'Active' },
            { label: 'MW Installed', value: '500+', icon: 'BarChart3', color: 'text-purple-500 bg-purple-50', status: 'Active' }
        ],
        testimonials: [
            { name: 'Rajesh Kumar', company: 'SunPower Solutions', position: 'CEO', testimonial: "This ERP system has transformed our solar business completely. We've seen a 40% increase in operational efficiency and better project management.", status: 'Active' },
            { name: 'Priya Sharma', company: 'Green Energy Systems', position: 'Operations Director', testimonial: "The solar-specific features like panel performance tracking and installation scheduling have made our workflow seamless. Highly recommended!", status: 'Active' },
            { name: 'Amit Patel', company: 'SolarTech India', position: 'Founder', testimonial: "From lead management to project completion, everything is integrated. The dealer app is a game-changer for our business.", status: 'Active' },
            { name: 'Neha Gupta', company: 'EcoSun Enterprises', position: 'Project Manager', testimonial: "The ROI calculator and solar design tools help us provide accurate quotes to customers. Customer satisfaction has improved significantly.", status: 'Active' },
            { name: 'Vikram Singh', company: 'SolarMax Industries', position: 'Director', testimonial: "The AMC management and installer marketplace have revolutionized how we handle maintenance contracts. Excellent platform!", status: 'Active' }
        ],
        trustedCompanies: ['Tata Power Solar', 'Adani Green', 'Waaree', 'Vikram Solar', 'Solex Energy', 'Renew Power']
    });

    const [erpBenefitsConfig, setErpBenefitsConfig] = useState({
        sectionTitle: 'Benefits of Our ERP System',
        enableSection: true,
        benefits: [
            { icon: 'TrendingUp', title: 'Increased Efficiency', desc: 'Automate manual processes and reduce operational costs by up to 30%', color: 'bg-purple-50 text-purple-700', status: 'Active' },
            { icon: 'Eye', title: 'Better Visibility', desc: 'Real-time insights into all business operations and performance metrics', color: 'bg-indigo-50 text-indigo-700', status: 'Active' },
            { icon: 'ArrowLeftRight', title: 'Streamlined Operations', desc: 'Seamless data flow between departments eliminating silos', color: 'bg-blue-50 text-blue-700', status: 'Active' },
            { icon: 'ClipboardList', title: 'Improved Compliance', desc: 'Automated compliance tracking and audit trails', color: 'bg-green-50 text-green-700', status: 'Active' },
            { icon: 'Wallet', title: 'Cost Reduction', desc: 'Reduce IT costs, eliminate redundant systems, and optimize resources', color: 'bg-orange-50 text-orange-700', status: 'Active' }
        ],
        rightCard: {
    ...
    useEffect(() => {
        fetch(`${BASE_URL}/api/website/v1/hero-section/get?t=${Date.now()}`, { cache: 'no-store' })
            .then((res) => res.json())
            .then((data) => {
                if (data?.success && data?.data) {
                    setHeroSectionConfig((prev) => ({ ...prev, ...data.data }));
                }
            })
            .catch((err) => {
                console.error('Failed to fetch hero section config:', err);
            });
        ...
        fetch(`${BASE_URL}/api/website/v1/get?t=${Date.now()}`, { cache: 'no-store' })
            ...
            .then((data) => {
                if (data?.success && data?.data) {
                    ...
                    if (data.data.erpModulesSubTitle) {
                        setErpModulesConfig((prev) => ({
                            ...prev,
                            subTitle: data.data.erpModulesSubTitle
                        }));
                    }
                }
            })
        ...
        fetch(`${BASE_URL}/api/website/v1/erp-modules/get?t=${Date.now()}`, { cache: 'no-store' })
            .then((res) => res.json())
            .then((data) => {
                if (data?.success && data?.data && data.data.menuTitle) {
                    setErpModulesConfig(data.data);
                }
            })
            .catch((err) => {
                console.error('Failed to fetch ERP Modules config:', err);
            });

        fetch(`${BASE_URL}/api/website/v1/happy-users/get?t=${Date.now()}`, { cache: 'no-store' })
            .then((res) => res.json())
            .then((data) => {
                if (data?.success && data?.data) {
                    setHappyUsersConfig(data.data);
                }
            })
            .catch((err) => {
                console.error('Failed to fetch Happy Users config:', err);
            });

        fetch(`${BASE_URL}/api/website/v1/erp-benefits/get?t=${Date.now()}`, { cache: 'no-store' })
            .then((res) => res.json())
            .then((data) => {
                if (data?.success && data?.data) {
                    setErpBenefitsConfig((prev) => ({
                        ...prev,
                        ...data.data
                    }));
                }
            ...
            .catch((err) => {
                console.error('Failed to fetch ERP Benefits config:', err);
            });
    ...
    const renderModuleIcon = (iconVal, title, textClass = 'text-purple-600') => {
        if (typeof iconVal === 'string' && (iconVal.startsWith('http://') || iconVal.startsWith('https://') || iconVal.startsWith('data:') || iconVal.startsWith('/'))) {
            return <img src={iconVal} alt={title || 'Icon'} className="h-6 w-6 object-contain" />;
        }
        const IconComp = (typeof iconVal === 'string' ? ICON_MAP[iconVal] : iconVal) || Briefcase;
        return <IconComp className={`h-6 w-6 ${textClass}`} />;
    };

    const defaultFallbackModules = [
        { icon: Wallet, title: "Finance & Accounting", desc: "General ledger, accounts payable/receivable, budgeting, and financial reporting", status: "Active", order: 1 },
        { icon: Boxes, title: "Inventory Management", desc: "Stock control, warehouse management, batch tracking, and reorder management", status: "Active", order: 2 },
        { icon: Users, title: "HR Management", desc: "Employee records, attendance, leave management, and performance tracking", status: "Active", order: 3 },
        { icon: CreditCard, title: "Payroll", desc: "Salary processing, tax calculations, deductions, and payslip generation", status: "Active", order: 4 },
        { icon: ShoppingCart, title: "Procurement", desc: "Purchase orders, vendor management, RFQs, and contract management", status: "Active", order: 5 },
        { icon: Factory, title: "Production", desc: "Manufacturing planning, BOM, work orders, and quality control", status: "Active", order: 6 },
        { icon: Users, title: "Sales & CRM", desc: "Lead management, quotations, sales orders, and customer relationship", status: "Active", order: 7 },
        { icon: BarChart3, title: "Business Intelligence", desc: "Advanced analytics, dashboards, reports, and predictive insights", status: "Active", order: 8 },
        { icon: ClipboardList, title: "Project Management", desc: "Project planning, resource allocation, task tracking, and time sheets", status: "Active", order: 9 },
        { icon: Truck, title: "Supply Chain", desc: "Logistics, shipment tracking, fleet management, and route optimization", status: "Active", order: 10 },
        { icon: HelpCircle, title: "Customer Support", desc: "Ticket system, service requests, warranty management, and feedback", status: "Active", order: 11 },
        { icon: ShieldCheck, title: "Compliance", desc: "Regulatory compliance, audit trails, document management, and security", status: "Active", order: 12 }
    ];

    const rawModulesList = Array.isArray(erpModulesConfig.modules) && erpModulesConfig.modules.length > 0
        ? erpModulesConfig.modules
        : defaultFallbackModules;

    const displayModules = rawModulesList
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            {/* Hero Section */}
            {heroSectionConfig.enableSection !== false && (
                <section className="relative w-full bg-gradient-to-br from-blue-50 via-white to-orange-50 px-8 py-16 md:px-16 lg:px-24">
                    <div className="mx-auto flex max-w-7xl flex-col items-center gap-12 lg:flex-row">
                        ...
                        <div className="flex-1 text-left">
                            {/* Welcome badge */}
                            {(heroSectionConfig.badge || headerData.badge) && (
                                <div className="inline-flex items-center space-x-2 rounded-full bg-gradient-to-r from-orange to-deep-orange px-4 py-2 text-white shadow-lg shadow-orange/30">
                                    ...
                                    <span className="text-sm font-semibold">{heroSectionConfig.badge || headerData.badge}</span>
                                </div>
                            )}

                            <h1 className="mt-8 text-4xl font-bold tracking-tight text-gray-800 md:text-5xl lg:text-6xl">
                                {heroSectionConfig.title || headerData.title || 'Powering a Sustainable Future with Smart Solar Solutions'}
                            </h1>
                            {(heroSectionConfig.subtitle || headerData.subtitle) && (
                                <p className="mt-2 text-2xl font-bold text-primary">
                                    {heroSectionConfig.subtitle || headerData.subtitle}
                                </p>
                            )}
                            {(heroSectionConfig.description || headerData.description) && (
                        ...
                        <div className="flex-1 w-full max-w-xl">
                            <div className="rounded-2xl bg-white p-6 md:p-8 shadow-xl border border-gray-100 flex flex-col items-center justify-center min-h-[400px] overflow-hidden">
                                {(heroSectionConfig.imageUrl || headerData.imageUrl) ? (
                                    <img
                                        src={heroSectionConfig.imageUrl || headerData.imageUrl}
                                        alt="Hero Banner"
                                        className="max-h-[350px] w-full object-cover rounded-xl shadow-sm transition-all duration-300"
                                    />
                                ) : (
                                    <>
                                        ...
                                        <h2 className="mt-6 text-3xl font-black tracking-wide text-primary">
                                            {heroSectionConfig.subtitle || headerData.subtitle || 'ERP System'}
                                        </h2>
                                    </>
                                )}
                            </div>
            ...
            {/* Key ERP Modules Section */}
            {erpModulesConfig.enableSection !== false && (
                <section id="modules" className="w-full bg-gradient-to-br from-blue-50 via-white to-orange-50 px-8 py-20 md:px-16 lg:px-24">
                    <div className="mx-auto max-w-7xl text-center">
                        <h2 className="text-4xl font-bold tracking-tight text-gray-800">
                            {erpModulesConfig.menuTitle || 'Comprehensive ERP Modules'}
                        </h2>
                        <p className="mt-4 text-lg text-gray-600">
                            {erpModulesConfig.subTitle || 'Everything you need to run your business efficiently'}
                        </p>
                        ...
                        <div className="mt-12 grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                            {displayModules.map((m, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div>
                                            <div className={`inline-flex rounded-lg p-3 ${bgClass} ${textClass}`}>
                                                {renderModuleIcon(m.icon || m.logo, m.title, textClass)}
                                            </div>
            ...
            {/* Our Users Section */}
            {happyUsersConfig.enableSection !== false && (
                <section className="w-full bg-gradient-to-br from-orange-50 via-white to-blue-50 px-8 py-20 md:px-16 lg:px-24">
                    <div className="mx-auto max-w-7xl text-center">
                        <h2 className="text-4xl font-bold tracking-tight text-gray-800">
                            {happyUsersConfig.sectionTitle || 'Our Happy Users'}
                        </h2>
                        <p className="mt-4 text-lg text-gray-600">
                            {happyUsersConfig.subTitle || 'Trusted by solar businesses across India'}
                        </p>

                        {/* User Stats Row */}
                        {Array.isArray(happyUsersConfig.stats) && happyUsersConfig.stats.length > 0 && (
                            <div className="mt-12 grid gap-6 grid-cols-2 lg:grid-cols-4">
                                {happyUsersConfig.stats
                                    ...
                                    .map((stat, idx) => {
                                        return (
                                            ...
                                            >
                                                <div className={`rounded-full p-4 ${stat.color || 'text-blue-500 bg-blue-50'}`}>
                                                    {renderModuleIcon(stat.icon, stat.label, '')}
                                                </div>
                        ...
                        {Array.isArray(happyUsersConfig.testimonials) && happyUsersConfig.testimonials.filter((t) => t.status !== 'Inactive').length > 0 && (
                            <div className="mt-16 w-full">
                                <Carousel
                                    items={happyUsersConfig.testimonials
                        ...
                        {/* User Companies */}
                        {Array.isArray(happyUsersConfig.trustedCompanies) && happyUsersConfig.trustedCompanies.length > 0 && (
                            <div className="mt-20">
                                ...
                                <div className="mt-8 flex flex-wrap justify-center items-center gap-12 opacity-60">
                                    {happyUsersConfig.trustedCompanies.map((c) => (
            ...
            {erpBenefitsConfig.enableSection !== false && (
                    ...
                    <div className="mx-auto max-w-7xl flex flex-col items-center gap-12 lg:flex-row">
                        ...
                        <div className="flex-1 text-left space-y-8">
                            <h2 className="text-4xl font-extrabold text-gray-800 tracking-tight">
                                {erpBenefitsConfig.sectionTitle || 'Benefits of Our ERP System'}
                            </h2>

                            <div className="space-y-6">
                                {(erpBenefitsConfig.benefits || [])
                                    .filter((item) => item.status === 'Active')
                                    .map((item, idx) => {
                                        const IconComp = ICON_MAP[item.icon] || TrendingUp;
                                        return (
                                            <div key={item.id || idx} className="flex gap-4 items-start">
                                                <div className={`rounded-full p-3 flex-shrink-0 ${item.color || 'bg-purple-50 text-purple-700'}`}>
                        ...
                        <div className="flex-1 w-full max-w-xl">
                            <div className="h-[420px] w-full rounded-[32px] bg-gradient-to-br from-blue-50/60 via-white to-orange-50/60 flex flex-col items-center justify-center p-8 shadow-xl relative overflow-hidden">
                                {(() => {
                                    const CardIconComp = ICON_MAP[erpBenefitsConfig.rightCard?.icon] || TrendingUp;
                                    return <CardIconComp className="h-28 w-28 text-purple-300 animate-pulse" />;
                                })()}
                                <h3 className="mt-6 text-2xl font-black text-purple-400 tracking-wide uppercase">
                                    {erpBenefitsConfig.rightCard?.title || 'Business Growth'}
                                </h3>
                            </div>
                        </div>
