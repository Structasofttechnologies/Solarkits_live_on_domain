 ...
 *
 * Cinematic Featured Media Hero Banner for BOS Kits Distributor.
 */

import React, { useRef, useEffect, useState } from 'react';
import { FiPlay, FiMaximize2, FiArrowRight, FiClock } from 'react-icons/fi';
...
export default function FeaturedMediaHero({
  ...
  const posterUrl = primaryMedia?.poster_url || primaryMedia?.thumbnail_url || imageMedia?.url || posterMedia?.url || DEFAULT_POSTERS.commercial;
  ...
  const badgeLabel = isVideoItem
    ? 'Technical 4K Demo'
    : content_type === 'POSTER'
    ? 'Component Spec Poster'
    : 'BOS Industry Showcase';
  ...
  return (
    <div
      ...
      role="region"
      aria-label={heading || 'Featured BOS Showcase'}
    >
      ...
      <div className="absolute inset-0 w-full h-full bg-slate-950 overflow-hidden">
        {isVideoItem && activeVideoUrl && !videoError ? (
          ...
          <img
            src={posterUrl}
            alt={heading || 'Featured BOS Showcase'}
            className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"
            style={{ objectPosition: focal_position }}
          />
        )}
        ...
        <div className="absolute inset-0 z-15 flex items-center justify-center pointer-events-none">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/40 text-white flex items-center justify-center shadow-2xl transform group-hover:scale-110 transition-all duration-300 pointer-events-auto">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white text-slate-900 flex items-center justify-center shadow-lg">
              <FiPlay size={24} className="ml-1 text-[#0575B8]" />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative z-20 p-6 sm:p-8 sm:pb-8 flex flex-col justify-end mt-24 sm:mt-36 max-w-4xl space-y-2.5">
        <h1 className="font-heading font-black text-2xl sm:text-4xl text-white tracking-tight leading-tight drop-shadow-md">
          {heading || content.title || 'BOS Hardware & Inverter Mounting Systems'}
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          {isVideoItem && (
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-[#0575B8] hover:bg-[#045D93] text-white text-xs font-black shadow-lg shadow-blue-500/30 transition-all active:scale-95 cursor-pointer"
            >
