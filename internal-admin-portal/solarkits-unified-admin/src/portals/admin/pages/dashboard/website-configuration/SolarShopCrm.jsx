...
import { UserPlus, FileText, CheckSquare, Layers, HelpCircle, Warehouse, Users, Sparkles } from "lucide-react";
...
export default function SolarShopCrm() {
      ...
      if (response.data?.data) {
        ...
        setFormData((prev) => {
          ...
          for (const key in data) {
            if (key === "crmList") continue;
            if (data[key] !== undefined && data[key] !== null && data[key] !== "") {
              merged[key] = data[key];
            }
          }
          if (Array.isArray(data.crmList) && data.crmList.length > 0) {
            merged.crmList = data.crmList.map(item => ({
              ...item,
  ...
  const handleSaveModalSolution = (e) => {
    ...
    if (editingCard !== null) {
      ...
      setFormData((prev) => {
        const updatedList = prev.crmList.map((c, idx) =>
          idx === editingCard
            ? {
                ...c,
        ...
        return { ...prev, crmList: updatedList };
      });
    } else {
      // Add new card
      const newCard = {
  ...
  const handleDeleteCard = (index) => {
    if (window.confirm("Are you sure you want to delete this card?")) {
      const updatedList = formData.crmList.filter((_, idx) => idx !== index);
      setFormData((prev) => ({
        ...prev,
        crmList: updatedList
      }));
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to reset CRM Modules fields to default values?")) {
      setFormData({
        ...formData,
        ...DEFAULT_CONFIG,
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-text/60 font-semibold text-sm">Loading CRM Modules Configuration...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader
        title="Solar Shop - CRM Modules"
        description="Configure titles, subtitles, card names, descriptions, metric badges, colors, and icons for the CRM Modules section"
      />
          ...
          <div className="border border-border/40 rounded-xl p-5 bg-bg/20 space-y-4">
            <div className="flex items-center justify-between border-b border-border/25 pb-3 mb-1">
              <h3 className="text-sm font-bold text-text flex items-center gap-2">
                <FiLayers className="text-primary" /> CRM Modules Headers
              </h3>
              <button
                type="button"
            ...
            <div>
              <label className="text-xs font-semibold text-text/80 block mb-1.5 uppercase tracking-wider">CRM Title</label>
              <input
                type="text"
                name="crmTitle"
                value={formData.crmTitle}
            ...
            <div>
              <label className="text-xs font-semibold text-text/80 block mb-1.5 uppercase tracking-wider">CRM Subtitle</label>
              <textarea
                rows={3}
                name="crmSubtitle"
                value={formData.crmSubtitle}
                onChange={handleChange}
                required
          ...
          <div className="bg-card border border-border/40 rounded-2xl p-5 shadow-xl bg-bg/20 space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-border/25">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold text-text">Modules List</h2>
                  <p className="text-xs text-text/60">Configure the interactive CRM modules cards</p>
                </div>
              </div>
              <button
                type="button"
                ...
                <tbody className="divide-y divide-border/40 font-medium">
                  {formData.crmList.map((card, idx) => (
                    <tr key={idx} className="hover:bg-primary/5 transition-colors group">
                      ...
                      <td className="py-3 px-4 text-text/60 font-semibold text-[11px]">
                        {card.badge || "-"}
                      </td>
                      <td className="py-3 px-4 text-text/65">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-bg/50 border border-border/40 rounded text-[10px]">
                          {card.icon} ({card.color.includes("blue") ? "Blue" : card.color.includes("green") ? "Green" : card.color.includes("purple") ? "Purple" : card.color.includes("orange") ? "Orange" : card.color.includes("red") ? "Red" : card.color.includes("teal") ? "Teal" : "Indigo"})
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        <button
                          ...
                          onClick={() => {
                            setFormData(prev => ({
                              ...prev,
                              crmList: prev.crmList.map((item, i) =>
                    ...
                    {formData.crmList.map((sol, i) => {
                      ...
                      return (
                        <div key={i} className="bg-white border border-gray-150 p-4 rounded-xl text-left shadow-sm">
                          ...
                          <span className="text-[8px] font-bold text-slate-400 block mt-0.5 tracking-wider uppercase">{sol.badge}</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-lg font-extrabold text-text mb-1">
                {editingCard !== null ? "Edit CRM Module" : "Add CRM Module"}
              </h3>
              <p className="text-xs text-text/60 mb-5">
                {editingCard !== null ? "Modify the properties of this CRM module card." : "Create a new CRM module card configuration."}
              </p>
              ...
              <form onSubmit={handleSaveModalSolution} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setModalForm((prev) => ({ ...prev, badge: e.target.value }))}
                    placeholder="e.g., 2,500+ LEADS/MONTH"
                    required
                    className="w-full bg-bg/50 border border-border/70 rounded-xl px-4 py-2.5 text-xs font-semibold text-text focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Provide a short module description..."
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      <option value="UserPlus">UserPlus Icon</option>
                      <option value="FileText">FileText Icon</option>
                      <option value="CheckSquare">CheckSquare Icon</option>
                      <option value="Layers">Layers Icon</option>
                      <option value="HelpCircle">HelpCircle Icon</option>
                      <option value="Warehouse">Warehouse Icon</option>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="from-teal-500/20 to-teal-600/20 text-teal-600">Teal Theme</option>
                      <option value="from-indigo-500/20 to-indigo-600/20 text-indigo-600">Indigo Theme</option>
                    </select>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {savedSuccess && (
          ...
          >
            ...
            <div>
              <h4 className="font-bold text-sm">CRM Configurations Saved</h4>
              <p className="text-xs text-white/90">Solar Shop CRM page updated successfully</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
