...
export default function SolarDealerAppHero() {
  ...
  useEffect(() => {
    fetchDealerAppConfig();
  }, []);

  const fetchDealerAppConfig = async () => {
    setLoading(true);
    try {
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to reset Dealer App fields to default values?")) {
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader
        title="Dealer App - Hero Section"
        description="Configure titles, descriptions, Play Store link, and hero banner logo of the Dealer App page"
      />
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-7 bg-card border border-border/40 shadow-xl rounded-2xl p-6 space-y-6">
          ...
          <div className="border border-border/40 rounded-xl p-5 bg-bg/20 space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-text/80 block mb-1.5 uppercase tracking-wider">Google Play download link</label>
              <input
                type="text"
                name="downloadLink"
            ...
            <div className="pt-2">
              <label className="text-xs font-semibold text-text/80 block mb-2 uppercase tracking-wider">Hero Section Image / Logo</label>
              ...
              >
                <div className={`flex items-center gap-6 ${previewMode === "mobile" ? "flex-col" : "flex-row"}`}>
                  <div className="flex-1 text-left space-y-4">
                    ...
                    <div className="flex items-center space-x-3 rounded-2xl border border-orange bg-transparent text-orange px-4 py-2 font-bold text-[9px] w-fit">
                      <div className="bg-white p-0.5 rounded flex items-center justify-center border border-gray-100">
                        <img 
                          src="/playstore.jpg" 
                          alt="Google Play" 
                    ...
                    <div className="border border-orange-100 bg-orange-50/40 p-4 rounded-xl flex flex-col items-center justify-center shadow-md min-h-[120px]">
                      {formData.imageUrl && formData.imageUrl !== "/logo.png" ? (
                        ...
                        <>
                          ...
                          <span className="text-[10px] font-black text-gray-800 tracking-tight">SolarKits</span>
                        </>
                      )}
                    </div>
                  </div>
      ...
      <AnimatePresence>
        {savedSuccess && (
          ...
          >
            ...
            <div>
              <h4 className="font-bold text-sm">Dealer App Saved</h4>
              <p className="text-xs text-white/90">Dealer App Hero updated successfully</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
