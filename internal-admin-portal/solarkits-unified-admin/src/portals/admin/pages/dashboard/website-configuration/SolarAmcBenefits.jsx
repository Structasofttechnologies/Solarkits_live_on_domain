...
const DEFAULT = {
  ...
  benefitsSubtitle: "Why businesses choose our AMC platform",
  benefitsList: [
    { title: "Maximize Efficiency",   description: "Automate scheduling and billing to save administrative hours.",                        enabled: true },
    { title: "Improve Uptime",        description: "Routine preventative testing minimizes unexpected panel/inverter breakdowns.",          enabled: true },
    { title: "Customer Satisfaction", description: "Rapid SLA resolution pipelines satisfy dealers and household clients.",                 enabled: true },
    { title: "Business Growth",       description: "Scale maintenance contract volume without enlarging operational personnel.",            enabled: true }
  ],
  enableSection: true
};
...
export default function SolarAmcBenefits() {
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      if (data?.data) {
        setFormData(prev => {
          const merged = { ...prev };
          if (data.data.benefitsTitle) merged.benefitsTitle = data.data.benefitsTitle;
          if (data.data.benefitsSubtitle) merged.benefitsSubtitle = data.data.benefitsSubtitle;
          if (Array.isArray(data.data.benefitsList) && data.data.benefitsList.length > 0) {
            merged.benefitsList = data.data.benefitsList.map(item => ({
              ...item,
              enabled: item.enabled !== undefined ? item.enabled : true
            }));
          }
          if (data.data.enableBenefitsSection !== undefined && data.data.enableBenefitsSection !== null) merged.enableSection = data.data.enableBenefitsSection;
          return merged;
        });
      }
  ...
  const handleSaveModal = e => {
    e.preventDefault();
    if (editingIdx !== null) {
      setFormData(prev => ({ ...prev, benefitsList: prev.benefitsList.map((b, i) => i === editingIdx ? { ...modalForm } : b) }));
    } else {
      setFormData(prev => ({ ...prev, benefitsList: [...prev.benefitsList, { ...modalForm, enabled: true }] }));
    }
  ...
  const handleSubmit = async e => {
    ...
    try {
      await axios.patch(`${BASE_URL}/api/website/v1/amc/update`, { ...formData, enableBenefitsSection: formData.enableSection, lastUpdated: new Date().toLocaleDateString('en-IN') });
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader title="AMC - Key Benefits" description="Configure the benefits section title and the benefits list" />
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <form onSubmit={handleSubmit} className="lg:col-span-7 bg-card border border-border/40 shadow-xl rounded-2xl p-6 space-y-5">
          <div className="border border-border/40 rounded-xl p-5 bg-bg/20 space-y-4">
            ...
            {[{ label: "Title", name: "benefitsTitle" }, { label: "Subtitle", name: "benefitsSubtitle" }].map(({ label, name }) => (
              <div key={name}>
          ...
          <div className="bg-bg/20 border border-border/40 rounded-2xl p-5">
            <div className="flex items-center justify-between pb-4 border-b border-border/25 mb-4">
              <div className="flex items-center gap-3"><div className="p-2.5 rounded-xl bg-primary/10 text-primary"><FiList className="text-xl" /></div><div><h2 className="text-sm font-bold text-text">Benefits</h2><p className="text-xs text-text/60">Checklist items</p></div></div>
            ...
            <div className="overflow-x-auto rounded-xl border border-border/50">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border/60 bg-bg/60 text-text/50 uppercase font-black text-[10px]"><th className="py-3 px-4">Title</th><th className="py-3 px-4">Description</th><th className="py-3 px-4 text-center">Status</th><th className="py-3 px-4 text-center">Actions</th></tr></thead>
                <tbody className="divide-y divide-border/40">
                  {formData.benefitsList.map((b, idx) => (
                    <tr key={idx} className="hover:bg-primary/5 transition-colors">
                      <td className="py-3 px-4 font-bold text-text">{b.title}</td>
                      <td className="py-3 px-4 text-text/60 max-w-[220px] truncate">{b.description}</td>
                      <td className="py-3 px-4 text-center">
                        <button
                          ...
                          onClick={() => {
                            setFormData(prev => ({
                              ...prev,
                              benefitsList: prev.benefitsList.map((item, i) => i === idx ? { ...item, enabled: item.enabled === false ? true : false } : item)
                            }));
                            setSavedSuccess(false);
                          }}
                          className={`relative inline-flex items-center gap-1.5 px-2 py-1 rounded-full border transition-all duration-300 cursor-pointer select-none ${b.enabled !== false ? "bg-primary border-primary" : "bg-bg/60 border-border/60"}`}
                        >
                          <span className={`relative inline-block w-6 h-3 rounded-full transition-colors duration-300 ${b.enabled !== false ? "bg-white/30" : "bg-border/60"}`}>
                            <span className={`absolute top-0.5 left-0.5 w-2 h-2 rounded-full bg-white shadow-sm transition-transform duration-300 ${b.enabled !== false ? "translate-x-3" : "translate-x-0"}`} />
                          </span>
                          <span className={`text-[9px] font-bold transition-colors duration-300 ${b.enabled !== false ? "text-white" : "text-text/50"}`}>
                            {b.enabled !== false ? "Enabled" : "Disabled"}
                          </span>
                        </button>
                      </td>
                      <td className="py-3 px-4 text-center"><div className="flex items-center justify-center gap-2">
                        <button type="button" onClick={() => { setEditingIdx(idx); setModalForm({ ...b }); setIsModalOpen(true); }} className="p-1.5 text-primary hover:bg-primary/10 border border-primary/20 rounded-lg transition cursor-pointer"><FiEdit2 className="text-xs" /></button>
                        <button type="button" onClick={() => { if (window.confirm("Delete?")) { setFormData(p => ({ ...p, benefitsList: p.benefitsList.filter((_, i) => i !== idx) })); setSavedSuccess(false); } }} className="p-1.5 text-rose-500 hover:bg-rose-500/10 border border-rose-500/20 rounded-lg transition cursor-pointer"><FiTrash2 className="text-xs" /></button>
                      </div></td>
                    </tr>
                  ))}
                  {formData.benefitsList.length === 0 && <tr><td colSpan={3} className="text-center py-6 text-text/50">No benefits. Add one.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-card border border-border/40 rounded-2xl p-5 shadow-xl sticky top-6">
            ...
            <div className="flex justify-center bg-bg/35 border border-border/30 rounded-xl p-4 overflow-hidden">
              <div className={`bg-white text-gray-800 overflow-hidden shadow-inner ${previewMode === "mobile" ? "w-[340px] min-h-[380px]" : "w-full min-h-[300px]"}`}>
                <div className="p-5 flex items-start gap-5">
                  <div className="w-20 h-28 bg-orange-50 border border-orange-100 rounded-xl flex items-center justify-center flex-shrink-0"><CheckCircle className="w-8 h-8 text-orange-400" /></div>
                  <div className="flex-1">
                    <h2 className="text-sm font-extrabold text-gray-800">{formData.benefitsTitle}</h2>
                    <p className="text-[9px] text-gray-400 mb-3">{formData.benefitsSubtitle}</p>
                    <div className="space-y-2">
                      {formData.benefitsList.slice(0, 4).map((b, i) => (
                        <div key={i} className="flex gap-2 items-start">
                          <CheckCircle className="w-3.5 h-3.5 text-blue-600 flex-shrink-0 mt-0.5" />
                          <div><h3 className="text-[9px] font-bold text-gray-800">{b.title}</h3><p className="text-[8px] text-gray-400 line-clamp-2">{b.description}</p></div>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-card border border-border/80 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
              ...
              <h3 className="text-lg font-extrabold text-text mb-4">{editingIdx !== null ? "Edit Benefit" : "Add Benefit"}</h3>
      ...
      <AnimatePresence>
        {savedSuccess && (
          <motion.div initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} className="fixed bottom-6 right-6 z-55 flex items-center gap-3 bg-green-550 text-white px-5 py-3.5 rounded-xl shadow-2xl border border-white/10">
            <FiCheckCircle className="text-xl" /><div><h4 className="font-bold text-sm">Benefits Saved</h4><p className="text-xs text-white/90">AMC benefits updated successfully</p></div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
