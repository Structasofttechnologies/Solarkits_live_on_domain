...
import { FolderGit2, CalendarDays, IndianRupee, Cpu, Map, ShieldAlert, ShieldCheck, Users2, Files } from "lucide-react";
...
const DEFAULT_CONFIG = {
  ...
  featuresList: [
    { title: "Multi-Project Management", description: "Handle multiple mega watt projects simultaneously with centralized control", icon: "FolderGit2", enabled: true },
    { title: "Gantt Charts", description: "Advanced project scheduling with critical path analysis and dependencies", icon: "CalendarDays", enabled: true },
    { title: "Budget Tracking", description: "Real-time budget monitoring, cost control, and financial forecasting", icon: "IndianRupee", enabled: true },
    { title: "Resource Management", description: "Optimize equipment, labor, and material allocation across projects", icon: "Cpu", enabled: true },
    { title: "GIS Integration", description: "Geographic information system for site mapping and planning", icon: "Map", enabled: true },
    { title: "Risk Management", description: "Identify, assess, and mitigate project risks proactively", icon: "ShieldAlert", enabled: true },
    { title: "Quality Control", description: "Track quality metrics, inspections, and compliance standards", icon: "ShieldCheck", enabled: true },
    { title: "Stakeholder Portal", description: "Dedicated access for investors, contractors, and regulatory bodies", icon: "Users2", enabled: true },
    { title: "Document Management", description: "Secure repository for blueprints, contracts, and engineering documents", icon: "Files", enabled: true }
  ],
...
export default function SolarMegawattFeatures() {
  ...
  useEffect(() => {
    fetchMegawattFeatures();
  }, []);

  const fetchMegawattFeatures = async () => {
    setLoading(true);
    try {
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to reset features to default values?")) {
      setFormData({
        ...formData,
        ...DEFAULT_CONFIG,
      });
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader
        title="Megawatt - Features List"
        description="Configure titles, subtitles, feature cards, and custom icons of the Megawatt Enterprise Features grid"
      />
                ...
                <div className="p-6 bg-white text-center">
                  ...
                  <div className={`mt-6 grid gap-4 ${previewMode === "mobile" ? "grid-cols-1" : "grid-cols-2"}`}>
                    {formData.featuresList.map((sol, i) => {
                      const Icon = ICON_MAP[sol.icon] || FolderGit2;
                      return (
                        <div key={i} className="bg-white border border-gray-150 p-4 rounded-xl text-left shadow-sm">
                          <div className="rounded-full bg-orange/10 p-2 text-orange w-fit mb-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-xs text-text/60 mb-5">
                Configure feature card parameters.
              </p>
              ...
              <form onSubmit={handleSaveModalSolution} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Multi-Project Management"
                    required
                ...
                <div>
                  ...
                  >
                    <option value="FolderGit2">FolderGit2 Icon</option>
                    <option value="CalendarDays">CalendarDays Icon</option>
                    <option value="IndianRupee">IndianRupee Icon</option>
                    <option value="Cpu">Cpu Icon</option>
                    <option value="Map">Map Icon</option>
                    <option value="ShieldAlert">ShieldAlert Icon</option>
                    <option value="ShieldCheck">ShieldCheck Icon</option>
                    <option value="Users2">Users2 Icon</option>
                    <option value="Files">Files Icon</option>
                  </select>
                </div>
        ...
        {savedSuccess && (
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-white/90">Megawatt features updated successfully</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
