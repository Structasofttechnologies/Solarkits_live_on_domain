...
export default function UrbanCities({ moduleUniqueId }) {
    ...
    // helper to geocode a row when latitude/longitude are missing
    ...
    const isCityAlreadyManaged = (cityToCheck, currentExcelData, currentExcludedPoints, currentUrbanCities) => {
        ...
        // Check in already saved urban cities
        ...
        // Check in current staged excel data
        ...
        // Check in excluded points for the current district
    ...
    const handleIncludeNear = (index) => {
        setNearPoints((prev) => {
            ...
            // Check if already managed (saved, staged, or excluded)
            if (isCityAlreadyManaged(item, excelData, excludedPoints, urbanCities)) {
                ...
                // Remove from nearPoints if it's already managed elsewhere
            ...
            dispatch(
                setAlert({
                    message: `Successfully loaded ${inside.length} cities from Excel. ${near.length} nearby points require review. ${far.length} points were skipped. ${skippedFromUpload.length > 0
                        ? `${skippedFromUpload.length} cities were skipped because they are already managed (saved, staged, or excluded). `
        ...
        setConfirmationPopup({
            ...
            onConfirm: async () => {
                try {
                    ...
                    dispatch(setAlert({ message: `${excelData.length} cities have been successfully uploaded.`, type: "success" }));
        ...
        setConfirmationPopup({
            ...
            onConfirm: async () => {
                try {
                    ...
                    dispatch(setAlert({ message: `"${cityName}" successfully excluded from this district.`, type: 'success' }));
                    setConfirmationPopup(prev => ({ ...prev, isOpen: false }));
                } catch (err) {
                    console.error('Error excluding city:', err);
                    dispatch(setAlert({ message: err.response?.data?.message || 'Failed to exclude city.', type: 'error' }));
    ...
    useEffect(() => {
        if (!district || !stateData || !country) {
            ...
            dispatch(setAlert({ message: "Missing state or country. Redirected to setup location.", type: "error", }));
        }
    }, [stateData, country, navigate, dispatch, location]);
    ...
    useEffect(() => {
        ...
        const fetchUrbanCities = async () => {
            try {
                ...
                dispatch(setAlert({ message: "Failed to load urban cities.", type: "error", }));
            } finally {
    ...
    // Filter out excluded points that are already in urbanCities
    ...
    const fetchSavedUrbanCities = async () => {
        ...
        try {
            ...
            dispatch(setAlert({ message: "Failed to load saved cities.", type: "error" }));
        }
    };
    ...
    const fetchExcludedCities = async () => {
        ...
        try {
            ...
            setExcludedPoints(prev => {
                ...
                // Add newly fetched data with proper districtId and fromApi flag
                ...
                // Combine existing local exclusions with new API data, avoiding duplicates
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ✅ Saved Urban Cities View (show only when user doesn't have edit but has view) */}
