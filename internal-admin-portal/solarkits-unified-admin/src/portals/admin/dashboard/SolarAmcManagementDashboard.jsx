...
import Home from "@/pages/solar-amc-management/Home";
import AmcPlans from "@/pages/solar-amc-management/AmcPlans";
import EpcPlans from "@/pages/epc-plans/EpcPlans";
...
const menus = [
  [
    { name: "Dashboard", icon: <FaHome />, path: "/admin-panel/solar-amc-management/home" },
    {
      name: "Plans",
      icon: <FaFileContract />,
      path: "/admin-panel/solar-amc-management/plans",
      subMenu: [
        { name: "AMC Plans", icon: <FaFileContract />, path: "/admin-panel/solar-amc-management/amc-plans" },
      ]
    },
  ]
];

export default function SolarAmcManagementDashboard() {
    ...
    const checkProductMarket = async () => {
      try {
        ...
        if (res.data?.status === "success") {
          ...
          const foundProduct = allProducts.find(p => p.slug === "solar-amc-management");
          ...
          const slugIndex = parts.indexOf("solar-amc-management");
          if (slugIndex !== -1) {
            ...
            if (hasCountry) {
              ...
              if (!subPath) {
                navigate(`/admin-panel/solar-amc-management/${nextSegment.toLowerCase()}/home`, { replace: true });
              }
            } else {
              ...
              navigate(`/admin-panel/solar-amc-management/${defaultCountry}/${subPath}`, { replace: true });
            }
          }
  ...
  return (
    <div className="flex h-screen bg-bg theme-transition">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <Header isOpen={isOpen} setIsOpen={setIsOpen} isMobile={isMobile} title="Solar AMC Dashboard" />
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <Routes location={location}>
                  ...
                  <Route path="/amc-plans" element={<AmcPlans defaultCategory="All" />} />
                  <Route path="/:countryName/amc-plans" element={<AmcPlans defaultCategory="All" />} />
                  <Route path="/epc-plans" element={<EpcPlans />} />
                  <Route path="/:countryName/epc-plans" element={<EpcPlans />} />
                </Routes>
              </motion.div>
            </AnimatePresence>
          </div>
