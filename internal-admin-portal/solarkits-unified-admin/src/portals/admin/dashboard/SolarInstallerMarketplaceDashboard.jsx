...
import Home from "@/pages/solar-installer-marketplace/Home";
...
const menus = [
  [
    { name: "Dashboard", icon: <FaHome />, path: "/admin-panel/solar-installer-marketplace/home" },
  ]
];

export default function SolarInstallerMarketplaceDashboard() {
    ...
    const checkProductMarket = async () => {
      try {
        ...
        if (res.data?.status === "success") {
          ...
          const foundProduct = allProducts.find(p => p.slug === "solar-installer-marketplace");
          ...
          const slugIndex = parts.indexOf("solar-installer-marketplace");
          if (slugIndex !== -1) {
            ...
            if (hasCountry) {
              ...
              if (!subPath) {
                navigate(`/admin-panel/solar-installer-marketplace/${nextSegment.toLowerCase()}/home`, { replace: true });
              }
            } else {
              ...
              navigate(`/admin-panel/solar-installer-marketplace/${defaultCountry}/${subPath}`, { replace: true });
            }
          }
  ...
  return (
    <div className="flex h-screen bg-bg theme-transition">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <Header isOpen={isOpen} setIsOpen={setIsOpen} isMobile={isMobile} title="Solar Installer Marketplace Dashboard" />
