...
const EPCHomePage = lazy(() => import("@/pages/epc-project-management-erp/Home"));
  ...
  [
    {
      name: "Dashboard",
      icon: <MdDashboard />,
      path: "/admin-panel/epc-project-management-erp/dashboard",
    },
  ],
];

export default function EPCCompanyDashboard() {
    ...
    const checkProductMarket = async () => {
      try {
        ...
        if (res.data?.status === "success") {
          ...
          const foundProduct = allProducts.find(p => p.slug === "epc-project-management-erp");
          ...
          const slugIndex = parts.indexOf("epc-project-management-erp");
          if (slugIndex !== -1) {
            ...
            if (hasCountry) {
              ...
              if (!subPath) {
                navigate(`/admin-panel/epc-project-management-erp/${nextSegment.toLowerCase()}/dashboard`, { replace: true });
              }
            } else {
              ...
              navigate(`/admin-panel/epc-project-management-erp/${defaultCountry}/${subPath}`, { replace: true });
            }
          }
  ...
  return (
    <div className="flex h-screen bg-bg w-screen theme-transition">
      ...
      <div className="flex flex-col flex-1 min-w-0 max-w-full">
        <Header isOpen={isOpen} setIsOpen={setIsOpen} isMobile={isMobile} title="EPC Company Dashboard" />
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <Routes location={location}>
                  <Route path="/dashboard/*" element={<Suspense fallback={<Loader text="Loading dashboard..." />}><EPCHomePage /></Suspense>} />
                  <Route path="/:countryName/dashboard/*" element={<Suspense fallback={<Loader text="Loading dashboard..." />}><EPCHomePage /></Suspense>} />
                </Routes>
              </motion.div>
            </AnimatePresence>
          </div>
