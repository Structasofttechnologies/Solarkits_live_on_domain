...
export const getUserData = createAsyncThunk(
  ...
  async (_, { rejectWithValue, dispatch, getState }) => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_ACCOUNT_API_URL || import.meta.env.VITE_API_URL || 'http://localhost:5000/account-api'}/user-data`, {
...
const savedScope = localStorage.getItem('selected_scope');
...
const userSlice = createSlice({
  ...
  reducers: {
    logoutUser: (state) => {
      ...
      localStorage.removeItem('selected_scope');
    },
    setSelectedScope: (state, action) => {
      state.selectedScope = action.payload;
      localStorage.setItem('selected_scope', JSON.stringify(action.payload));
    }
  },
