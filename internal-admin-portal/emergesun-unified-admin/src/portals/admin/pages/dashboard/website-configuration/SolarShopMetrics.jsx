...
const DEFAULT_CONFIG = {
  ...
  metricsList: [
    { val: "100+", label: "SUCCESSFUL PROJECTS", enabled: true },
    { val: "99.9%", label: "SYSTEM UPTIME", enabled: true },
    { val: "4.9/5", label: "CLIENT RATING", enabled: true },
    { val: "20K Tons", label: "CARBON REDUCTION", enabled: true }
  ],
  enableSection: true,
};
...
export default function SolarShopMetrics() {
  ...
  useEffect(() => {
    fetchSolarShopMetrics();
  }, []);

  const fetchSolarShopMetrics = async () => {
    setLoading(true);
    try {
      ...
      if (response.data?.data) {
        ...
        setFormData((prev) => {
          ...
          if (Array.isArray(data.metricsList) && data.metricsList.length > 0) {
            merged.metricsList = data.metricsList.map(item => ({
              ...item,
  ...
  const handleSaveModalSolution = (e) => {
    e.preventDefault();
    if (!modalForm.val.trim()) return;

    if (editingCard !== null) {
      // Update existing card
      setFormData((prev) => {
        const updatedList = prev.metricsList.map((c, idx) =>
          idx === editingCard
            ? {
                ...c,
                val: modalForm.val.trim(),
                label: modalForm.label.trim().toUpperCase()
              }
            : c
        );
        return { ...prev, metricsList: updatedList };
      });
    } else {
      // Add new card
      const newCard = {
      ...
      setFormData((prev) => ({
        ...prev,
        metricsList: [...prev.metricsList, newCard]
      }));
    }
  ...
  const handleDeleteCard = (index) => {
    if (window.confirm("Are you sure you want to delete this metric?")) {
      const updatedList = formData.metricsList.filter((_, idx) => idx !== index);
      setFormData((prev) => ({
        ...prev,
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to reset Metrics fields to default values?")) {
      setFormData({
        ...formData,
        ...DEFAULT_CONFIG,
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-text/60 font-semibold text-sm">Loading Metrics Configuration...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader
        title="Solar Shop - Performance Metrics"
        description="Configure titles and numerical metric cards values for the performance section"
      />
          ...
          <div className="border border-border/40 rounded-xl p-5 bg-bg/20 space-y-4">
            <div className="flex items-center justify-between border-b border-border/25 pb-3 mb-1">
              <h3 className="text-sm font-bold text-text flex items-center gap-2">
                <FiLayers className="text-primary" /> Metrics Headers
              </h3>
              <button
                type="button"
            ...
            <div>
              <label className="text-xs font-semibold text-text/80 block mb-1.5 uppercase tracking-wider">Metrics Title</label>
              <input
                type="text"
                name="metricsTitle"
          ...
          <div className="bg-card border border-border/40 rounded-2xl p-5 shadow-xl bg-bg/20 space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-border/25">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold text-text">Metrics List</h2>
                  <p className="text-xs text-text/60">Configure the interactive statistics cards</p>
                </div>
              </div>
              <button
                type="button"
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-border/60 bg-bg/60 text-text/50 uppercase font-black tracking-wider text-[10px]">
                    ...
                    <th className="py-3 px-4">Metric Label / Description</th>
                ...
                <tbody className="divide-y divide-border/40 font-medium">
                  {formData.metricsList.map((card, idx) => (
                    <tr key={idx} className="hover:bg-primary/5 transition-colors group">
                      ...
                      <td className="py-3 px-4 font-bold text-primary text-sm">
                        {card.val || "-"}
                      </td>
                      <td className="py-3 px-4 text-text/60 font-semibold uppercase tracking-wide">
                        {card.label || "-"}
                      </td>
                      <td className="py-3 px-4 text-center">
                        <button
                          ...
                          onClick={() => {
                            setFormData(prev => ({
                              ...prev,
                              metricsList: prev.metricsList.map((item, i) =>
              ...
              >
                <div className="p-6 bg-white text-center">
                  ...
                  <h2 className="text-base font-extrabold text-gray-800 leading-tight">
                    {formData.metricsTitle}
                  </h2>

                  <div className={`mt-6 grid gap-4 grid-cols-2 ${previewMode === "mobile" ? "lg:grid-cols-2" : "lg:grid-cols-4"}`}>
                    {formData.metricsList.map((sol, i) => {
                      return (
                        <div key={i} className="bg-white border border-gray-150 p-4 rounded-xl text-center shadow-sm">
                          <h3 className="font-extrabold text-blue-600 text-base">{sol.val}</h3>
                          <p className="text-[7px] font-black text-gray-400 mt-1 uppercase tracking-wider">{sol.label}</p>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-lg font-extrabold text-text mb-1">
                {editingCard !== null ? "Edit Metric" : "Add Metric"}
              </h3>
              <p className="text-xs text-text/60 mb-5">
                {editingCard !== null ? "Modify the properties of this performance metric." : "Create a new performance metric card."}
              </p>
              ...
              <form onSubmit={handleSaveModalSolution} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={modalForm.val}
                    onChange={(e) => setModalForm((prev) => ({ ...prev, val: e.target.value }))}
                    placeholder="e.g., 100+ or 99.9%"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-text mb-1.5">
                    Metric Label / Description <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={modalForm.label}
                    onChange={(e) => setModalForm((prev) => ({ ...prev, label: e.target.value }))}
                    placeholder="e.g., SUCCESSFUL PROJECTS"
                    required
                    className="w-full bg-bg/50 border border-border/70 rounded-xl px-4 py-2.5 text-xs font-semibold text-text focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition"
                  />
                </div>
                ...
                <div className="pt-4 flex items-center justify-end gap-3">
                  ...
                  >
                    {editingCard !== null ? "Save Metric" : "Add Metric"}
                  </button>
                </div>
              </form>
            </motion.div>
      ...
      <AnimatePresence>
        {savedSuccess && (
          ...
          >
            ...
            <div>
              <h4 className="font-bold text-sm">Metrics Saved</h4>
