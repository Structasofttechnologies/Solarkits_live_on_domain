...
import { FileEdit, BadgeCent, ArrowRightLeft, Sun, ShieldCheck, GitCompare, Star, Lock, Headphones, Target } from "lucide-react";
...
const DEFAULT_CONFIG = {
  ...
  stepsList: [
    { num: "1", title: "Post Your Project", description: "Describe your solar requirements and preferences", icon: "FileEdit", enabled: true },
    { num: "2", title: "Receive Quotes", description: "Get competitive quotes from verified installers", icon: "BadgeCent", enabled: true },
    { num: "3", title: "Compare & Select", description: "Review quotes and choose the best installer", icon: "ArrowRightLeft", enabled: true },
    { num: "4", title: "Get Installed", description: "Schedule installation and enjoy solar energy", icon: "Sun", enabled: true }
  ],
...
export default function SolarShopMarketplaceSteps() {
  ...
  useEffect(() => {
    fetchMarketplaceSteps();
  }, []);

  const fetchMarketplaceSteps = async () => {
    setLoading(true);
    try {
      ...
      if (response.data?.data) {
        ...
        setFormData((prev) => {
          ...
          for (const key in data) {
            if (key === "stepsList") continue;
            if (data[key] !== undefined && data[key] !== null && data[key] !== "") {
              merged[key] = data[key];
            }
          }
          if (Array.isArray(data.stepsList) && data.stepsList.length > 0) {
            merged.stepsList = data.stepsList.map(item => ({
              ...item,
              enabled: item.enabled !== undefined ? item.enabled : true
            }));
          }
          if (data.enableStepsSection !== undefined) {
            merged.enableStepsSection = data.enableStepsSection;
          }
          return merged;
        });
      }
  ...
  const handleOpenAddModal = () => {
    setEditingCard(null);
    setModalForm({
      num: String(formData.stepsList.length + 1),
      title: "",
      description: "",
      icon: "FileEdit",
      enabled: true
  ...
  const handleSaveModalSolution = (e) => {
    ...
    if (editingCard !== null) {
      ...
      setFormData((prev) => {
        const updatedList = prev.stepsList.map((c, idx) =>
          idx === editingCard
            ? {
                ...c,
        ...
        return { ...prev, stepsList: updatedList };
      });
    } else {
      // Add new step
      const newCard = {
      ...
      setFormData((prev) => ({
        ...prev,
        stepsList: [...prev.stepsList, newCard]
      }));
    }
  ...
  const handleDeleteCard = (index) => {
    if (window.confirm("Are you sure you want to delete this step card?")) {
      const updatedList = formData.stepsList.filter((_, idx) => idx !== index);
      setFormData((prev) => ({
        ...prev,
        stepsList: updatedList
      }));
      setSavedSuccess(false);
    }
  };
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to reset How It Works steps fields to default values?")) {
      setFormData({
        ...formData,
        ...DEFAULT_CONFIG,
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-text/60 font-semibold text-sm">Loading Steps Configuration...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader
        ...
        description="Configure titles, workflow steps numbers, descriptions and custom icons for the Installer Marketplace page"
      />
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-7 bg-card border border-border/40 shadow-xl rounded-2xl p-6 space-y-6">
          ...
          <div className="border border-border/40 rounded-xl p-5 bg-bg/20 space-y-4">
            <div className="flex items-center justify-between border-b border-border/25 pb-3 mb-1">
              ...
              <button
                type="button"
                onClick={() => setFormData(prev => ({ ...prev, enableStepsSection: !prev.enableStepsSection }))}
                className={`relative flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all duration-300 cursor-pointer select-none ${formData.enableStepsSection ? "bg-primary border-primary" : "bg-bg/60 border-border/60"}`}
              >
                <span className={`relative inline-block w-8 h-4 rounded-full transition-colors duration-300 ${formData.enableStepsSection ? "bg-white/30" : "bg-border/60"}`}>
                  <span className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform duration-300 ${formData.enableStepsSection ? "translate-x-4" : "translate-x-0"}`} />
                </span>
                <span className={`text-xs font-bold transition-colors duration-300 ${formData.enableStepsSection ? "text-white" : "text-text/50"}`}>
                  {formData.enableStepsSection ? "Enabled" : "Disabled"}
                </span>
              </button>
            </div>
            <div>
          ...
          <div className="bg-card border border-border/40 rounded-2xl p-5 shadow-xl bg-bg/20 space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-border/25">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold text-text">Steps List</h2>
                  <p className="text-xs text-text/60">Configure the sequential guide cards</p>
                </div>
              </div>
              <button
                type="button"
              ...
              >
                <FiPlus className="text-sm" /> Add Step
              </button>
            </div>
            ...
            <div className="overflow-x-auto rounded-xl border border-border/50 bg-bg/20">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-border/40 font-medium">
                  {formData.stepsList.map((card, idx) => (
                      ...
                      <td className="py-3 px-4 text-center">
                        <button
                          ...
                          onClick={() => {
                            setFormData(prev => ({
                              ...prev,
                              stepsList: prev.stepsList.map((item, i) =>
            ...
            <div className="flex justify-center bg-bg/35 border border-border/30 rounded-xl p-4 overflow-hidden">
              <div
                className={`bg-white text-gray-800 transition-all duration-300 overflow-hidden shadow-inner ${
                  previewMode === "mobile" ? "w-[340px] min-h-[450px]" : "w-full min-h-[200px]"
                }`}
              ...
              >
                <div className="p-6 bg-white text-center">
                  ...
                  <div className={`mt-6 grid gap-4 ${previewMode === "mobile" ? "grid-cols-2" : "grid-cols-4"}`}>
                    {formData.stepsList.map((sol, i) => {
                      const Icon = ICON_MAP[sol.icon] || FileEdit;
                      return (
                        <div key={i} className="bg-white border border-gray-150 p-4 rounded-xl text-center shadow-sm relative">
                          ...
                          <Icon className="w-6 h-6 text-blue-600 mx-auto mb-2 mt-2" />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-lg font-extrabold text-text mb-1">
                {editingCard !== null ? "Edit Step Card" : "Add Step Card"}
              </h3>
              <p className="text-xs text-text/60 mb-5">
                {editingCard !== null ? "Modify the properties of this guide step." : "Create a new workflow step card configuration."}
              </p>
              ...
              <form onSubmit={handleSaveModalSolution} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-text mb-1.5">
                    Step Number <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={modalForm.num}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Post Your Project"
                    required
                    className="w-full bg-bg/50 border border-border/70 rounded-xl px-4 py-2.5 text-xs font-semibold text-text focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition"
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    <option value="FileEdit">FileEdit Icon</option>
                    <option value="BadgeCent">BadgeCent Icon</option>
                    <option value="ArrowRightLeft">ArrowRightLeft Icon</option>
                ...
                <div className="pt-4 flex items-center justify-end gap-3">
                  ...
                  >
                    {editingCard !== null ? "Save Step" : "Add Step"}
                  </button>
                </div>
              </form>
            </motion.div>
      ...
      <AnimatePresence>
        {savedSuccess && (
          ...
          >
            ...
            <div>
              <h4 className="font-bold text-sm">How It Works Saved</h4>
              <p className="text-xs text-white/90">Marketplace steps updated successfully</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
