...
export default function RuralCities({ moduleUniqueId }) {
        ...
        setConfirmationPopup({
            ...
            onConfirm: async () => {
                try {
                    ...
                    setAlert({ message: `${excelData.length} rural cities have been successfully uploaded.`, type: "success" });
        ...
        setConfirmationPopup({
            ...
            onConfirm: async () => {
                try {
                    ...
                    setAlert({ message: `"${cityName}" successfully excluded from this urban city.`, type: 'success' });
                    setConfirmationPopup(prev => ({ ...prev, isOpen: false }));
                } catch (err) {
    ...
    useEffect(() => {
        if (!urbanCity || !district || !stateData || !country) {
            ...
            setAlert({ message: "Missing location data. Redirected to setup location.", type: "error", });
        }
    }, [urbanCity, district, stateData, country, navigate, location.pathname]);
    ...
    useEffect(() => {
        ...
        const fetchRuralCities = async () => {
            try {
                ...
                setAlert({ message: "Failed to load rural cities.", type: "error", });
            } finally {
    ...
    }, [urbanCity?.id, API_URL, moduleUniqueId]);
    ...
    }, [district?.id, stateData?.name, country?.name, API_URL, moduleUniqueId]);
    ...
    const fetchSavedRuralCities = async () => {
        ...
        try {
            ...
            setAlert({ message: "Failed to load saved rural cities.", type: "error" });
        }
    };
    ...
    }, [savedFilteredData.length, savedItemsPerPage, savedTotalPages, savedCurrentPage]);
