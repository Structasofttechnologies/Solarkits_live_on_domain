                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    ...
                    <div className="flex gap-3 bg-primary/5 px-4 py-2 rounded-lg">
                        <div className="text-center">
                            ...
                            <div className="text-lg font-bold text-green-600">{excelData.length}</div>
                        </div>
                        <div className="text-center">
                            <div className="text-xs text-text-secondary">Nearby</div>
                            <div className="text-lg font-bold text-amber-600">{nearPoints.filter(p => p.status === 'near').length}</div>
                        </div>
                        <div className="text-center">
                            <div className="text-xs text-text-secondary">Excluded</div>
                            <div className="text-lg font-bold text-red-600">{excludedForDistrict.length}</div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="p-2 bg-white rounded border border-border sm:col-span-2">
                                ...
                                <p className={`font-semibold capitalize ${
                                    selectedMarker.status === 'included' ? 'text-green-600' :
                                    selectedMarker.status === 'excluded' ? 'text-red-600' :
                                    'text-amber-600'
                                }`}>
